...
public class BadgeCounter {
    ...
    static TextView mBadgeCount;
    static ImageView mIconBadge;
    ...
    public static void update(final Activity activity,final MenuItem menu, Drawable icon, int counter){

        if (counter == 0) {
            mBadgeCount.setVisibility(View.GONE);
            update(activity, menu, icon, counter);
        } else {
            if (counter > 0 && mBadgeCount!=null){
                mBadgeCount.setVisibility(View.VISIBLE);
                mBadgeCount.setText(counter);
                update(activity, menu, icon, counter);
            }
