...
public class SalesAdvancePayment_Adapter extends BaseAdapter implements Filterable{
    private ItemFilter mFilter = new ItemFilter();
    private ArrayList<PurchaseAdvancePayment> arraylist;

    private List<PurchaseAdvancePayment> paymentInvoiceList;
    private LayoutInflater layoutInflater;


    public SalesAdvancePayment_Adapter(Activity activity, List<PurchaseAdvancePayment> paymentInvoiceList) {
        super();
        this.paymentInvoiceList = paymentInvoiceList;
        layoutInflater = LayoutInflater.from(activity);
        this.arraylist = new ArrayList<PurchaseAdvancePayment>();
        this.arraylist.addAll(paymentInvoiceList);
    }
    ...
    public View getView(final int position, View convertView, final ViewGroup parent) {
        ...
        if (convertView == null) {
            convertView = layoutInflater.inflate(R.layout.adapter_sales_advancepaymentlist, parent, false);
        ...
        UtilView.showLogCat("afsa","Count "+getCount());
        PurchaseAdvancePayment paymentInvoice =paymentInvoiceList.get(position);
        ...
        if (paymentInvoice != null) {
            ...
            viewHolder.tvRemainingAmount.setText("" + paymentInvoice.getArbalance());
            if (paymentInvoice.getArbalance() > 0) {
                viewHolder.tvRemainingPayment.setVisibility(View.VISIBLE);
            } else {
                viewHolder.tvCreateInvoice.setVisibility(View.VISIBLE);
            }
            viewHolder.imageviewReturn.setVisibility(View.VISIBLE);
   ...
   /* public void filter(String charText) {
        ...
        if (charText.length() == 0) {
            paymentInvoiceList.addAll(arraylist);
        }
        else
        {
            for (SalesAdvancePayment wp : arraylist)
            {
                if (wp.getCustomerName().toLowerCase().contains(charText) || wp.getFormNo().toLowerCase().contains(charText) || String.valueOf(wp.getAmount()).toLowerCase().contains(charText)  ||String.valueOf(wp.getArbalance()).toLowerCase().contains(charText)  )
                {
                    paymentInvoiceList.add(wp);
                }
            }
        }
...
*/
    ...
    private class ItemFilter extends Filter {
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            final List<PurchaseAdvancePayment> list = paymentInvoiceList;
