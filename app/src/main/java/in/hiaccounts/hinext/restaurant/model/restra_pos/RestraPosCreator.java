...
public class RestraPosCreator {

    String TAG = RestraPosCreator.class.getSimpleName();
    private Map<Long, RestraPosCartItem> posCartItemMap = new HashMap<>();
    private int totalItems = 0;
    private double totalPrice = 0d;


    /**
     * Add a quantity of a certain {@link RestraPosCartItem} item to this pos cart
     *
     * @param posCartItem the item will be added to this pos cart
     * @param itemId           to be added
     */
    public void addItem(RestraPosCartItem posCartItem, Long itemId) {
     ...
     */
    public List<RestraPosCartItem> getItems() {
        List<RestraPosCartItem> posCartItems = new ArrayList<RestraPosCartItem>();
        if (posCartItemMap != null && posCartItemMap.size() > 0) {
            for (Map.Entry<Long, RestraPosCartItem> entry : posCartItemMap.entrySet()) {
    ...
    public void delete(List<SelectedItemsList> selectItemForDelete) {
        ...
        for (int i = 0; i < selectItemForDelete.size(); i++) {
            ...
            for (Iterator<Map.Entry<Long, RestraPosCartItem>> it = posCartItemMap.entrySet().iterator(); it.hasNext(); ) {
    ...
    public long checkItemQty(SelectedItemsList item){
        if (posCartItemMap.containsKey(item.getItemId())) {
              RestraPosCartItem cartItem = posCartItemMap.get(item.getItemId());
