 ...
 */
public class GTReceiptVoucherPdfImpl implements GTReceiptVoucherPdf {

    @Override
    public void generateReceiptVoucherPdf(Context context, SaveResponseData printData, OutputStream outPutStream, InputStream inputStream) throws IOException, DocumentException {
        ...
        Paragraph paragraph=new Paragraph(new Chunk(Constant.PDF_TITLE_GTRECEIPT, fontHeader));
        paragraph.setAlignment(Element.ALIGN_CENTER);
        paragraph.add(lineSeperator());

        String receiptNo="",companyDate="",prepraredBy="";
        if (printData.getOrNo()!=null){
            receiptNo=printData.getOrNo();
        }
      ...
      if (printData.getDateOfInvoice()!=null){
          companyDate=printData.getDateOfInvoice();
      }
        ...
        paragraph1.add(new Chunk(Constant.PDF_HEADER_GTBILLTO, fontHeader));
        paragraph1.setAlignment(Element.ALIGN_CENTER);

        Paragraph para=new Paragraph();
        if (printData.getCustomerName()!=null)
            para.add(new Chunk(new Chunk("Name   : "+printData.getCustomerName(),fontTax)));
...
*/
        ...
        if (printData.getDateOfInvoice()!=null)
            chunkList.add(new Chunk(printData.getDateOfInvoice(),fontTax));
        chunkList.add(new Chunk(Chunk.NEWLINE));
