 ...
 */
public class PosA4PurchaseReturnPdfImpl implements PosPurchaseReturnPdf {

    @Override
    public void generatePurchaseReturnInvoice(Context context, Checkout_ResponseData retailPrintDTO, OutputStream outPutStream, InputStream inputStream) throws IOException, DocumentException {
...
*/
        ...
        header.add(new Chunk(Constant.PDF_PURCHASE_TITLEPURCHASERETRUN, fontHeader));
       ...
       // Paragraph compnayInfo = new Paragraph();
        ...
        if (retailPrintDTO.getPiData()!=null){
            ...
            if (retailPrintDTO.getPiData().getSupplierInvNo() != null)
                billingInoChunks.add(new Chunk(new Chunk(retailPrintDTO.getPiData().getSupplierInvNo(), fontTax)));
            billingInoChunks.add(new Chunk(Chunk.NEWLINE));
