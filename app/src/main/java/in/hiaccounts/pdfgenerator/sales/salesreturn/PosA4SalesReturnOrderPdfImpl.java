...
import in.hiaccounts.hinext.checkout.model.sales.CompanyData;
 ...
 */
public class PosA4SalesReturnOrderPdfImpl implements PosSalesReturnOrderPdf {

    @Override
    public void generateSalesReturnOrder(Context context, Checkout_Sales_ResponseData retailPrintDTO, OutputStream outPutStream, InputStream inputStream) throws IOException, DocumentException {
        ...
        header.add(new Chunk(Constant.PDF_TITLE_SALESRETURNORDER, fontHeader));
        ...
        if (retailPrintDTO.getCompanyData() != null) {
            ...
            billingInoChunks.add(new Chunk(Constant.PDF_TITLE_SALESRETURNORDERNO + " : ", fontTaxBold));
            if (retailPrintDTO.getSiData().getSiNo() != null)
                billingInoChunks.add(new Chunk(new Chunk(retailPrintDTO.getSiData().getSiNo(), fontTax)));
            billingInoChunks.add(new Chunk(Chunk.NEWLINE));
   ...
   /*     Paragraph compnayInfo = new Paragraph();
        ...
        if (retailPrintDTO.getSiData() != null) {
            compnayInfo.add(new Phrase(new Chunk(Constant.PDF_TITLE_SALESRETURNORDERNO, fontTaxBold)));
            compnayInfo.add(new Chunk(" : ",fontTax));
            if (retailPrintDTO.getSiData().getSiNo() != null)
                compnayInfo.add(new Phrase(new Chunk(retailPrintDTO.getSiData().getSiNo(), fontTax)));
            compnayInfo.add(Chunk.NEWLINE);
        }*/

      /*  compnayInfo.add(new Phrase(new Chunk(Constant.PDF_CINFO_INVDATE, fontTaxBold)));
        compnayInfo.add(new Chunk(" : ",fontTax));
        if (retailPrintDTO.getDate() != null)
...

        {
            ...
            if (retailPrintDTO.getSiData().getSiNo() != null) {
                ...
                inventNo = retailPrintDTO.getSiData().getSiNo();
...

        {
            ...
            if (retailPrintDTO.getSiData().getSiNo() != null) {
                ...
                locationInNo = retailPrintDTO.getSiData().getSiNo();
        ...
        chunkList.add(new Chunk(Chunk.NEWLINE));
        chunkList.add(new Chunk(Constant.PDF_TITLE_SALESRETURNORDERNO + " : ", fontTaxBold));
        chunkList.add(new Chunk(inventNo, fontTax));
        chunkList.add(new Chunk(Chunk.NEWLINE));
        /*chunkList.add(new Chunk(Constant.PDF_BIILING_INVDATE+" : ", fontTaxBold));
        ...
        shiipedChunkList.add(new Chunk(Chunk.NEWLINE));
    /*    shiipedChunkList.add(new Chunk(Constant.PDF_SHIPPING_INVDATE+" : ", fontTaxBold));
        ...
        if (retailPrintDTO.getTotalExcludingTax() != 0)
            bottomTotalTable.addCell(getBottomTableCell(String.format("%.2f", retailPrintDTO.getTotalExcludingTax()), PdfPCell.ALIGN_RIGHT, fontTax));
