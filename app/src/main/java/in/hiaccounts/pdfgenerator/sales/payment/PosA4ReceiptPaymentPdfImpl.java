 ...
 */
public class PosA4ReceiptPaymentPdfImpl implements PosRecieptPaymentPdf {

    @Override
    public void generatePaymentPdf(Context context, PrintPaymentReceiptData printData, OutputStream outPutStream, InputStream inputStream) throws IOException, DocumentException {
        ...
        if (printData.getRegisterNo()!=null)
            companyGSTNo=printData.getRegisterNo();

        if (printData.getLocationId()!=null)
            companyRecieptNo=printData.getLocationId();
       ...
       /* Paragraph header1 = new Paragraph();
        ...
        if (printData.getCustomerName()!=null)
            customerName=printData.getCustomerName();
        ...
        billToC1List.add(new Chunk(Constant.PDF_CUSTOMERNAME,fontTax));
       ...
       /* Paragraph header2 = new Paragraph();
        ...
        if (printData.getSino()!=null)
            invNumber=printData.getSino();

        if (printData.getSidate()!=null)
            invDate=printData.getSidate();

        invAmt=""+printData.getTotalReceivable();
        invAmtPaid=""+printData.getTotalCashPayment();
